"""VideoMAE temporal-fusion training for binary score prediction."""
...
@dataclass
class VideoMaeTemporalFusionScorePredictionBinaryTrainCfg:
    task_name: str
    model_name: str
    backbone: str
...
def run_training(config: VideoMaeTemporalFusionScorePredictionBinaryTrainCfg) -> str:
    load_dotenv()
    ...
    label_mappings = load_target_label_mappings(config.vocab_path, "quality_label")
    ...
    class_weights = (
        build_class_weights(
            [label_transform(row["quality_label"]) for row in dataset["train"]],
...
def _training_params(
    config: VideoMaeTemporalFusionScorePredictionBinaryTrainCfg,
) -> dict[str, object]:
    return {
        "backbone": config.backbone,
        "frozen": True,
...
__all__ = ["VideoMaeTemporalFusionScorePredictionBinaryTrainCfg", "run_training"]
