"""VideoMAE temporal-fusion evaluation for ordinal score prediction."""
...
from aitraf.tasks.score_prediction_ordinal.metrics import (
    amae,
...
@dataclass
class VideoMaeTemporalFusionScorePredictionOrdinalEvalCfg:
    model_uri: str
    backbone: str
...
def run_evaluation(config: VideoMaeTemporalFusionScorePredictionOrdinalEvalCfg) -> None:
    load_dotenv()
    ...
    with mlflow.start_run(run_name=config.run_name):
        ...
        metrics_report = calc_metrics_for_models(
            eval_models=[
                EvalModel(
                    ...
                    sets=[
                        EvalSet(
                            name="train",
                            predictions=compute_constant_median_pred_ids(
                                train_labels,
                                len(train_labels),
                            ),
                            labels=train_labels,
            ...
            eval_metrics=(amae, mae, mmae, qwk),
        )
...
__all__ = ["VideoMaeTemporalFusionScorePredictionOrdinalEvalCfg", "run_evaluation"]
