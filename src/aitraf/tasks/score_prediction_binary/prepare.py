"""Task-owned data preparation for binary score prediction."""
...
STRATIFY_COL = "quality_label"
STRATIFY_STRATEGY = "label"
VOCAB_COLUMNS = ("quality_label", "trick", "key_foot", "person")
MANIFEST_COLUMNS = (
    "video_id",
    "s3_path",
    "quality_label",
...
def run_prepare(task_cfg: DictConfig, prepare_cfg: DictConfig) -> None:
    """Prepare manifests for binary score prediction."""
    ...
    filtered_df = filtered_df.loc[
        filtered_df["execution_score"].isin(QUALITY_LABELS)
...
def _build_manifest_df(labels_df: pd.DataFrame) -> pd.DataFrame:
    ...
    manifest_df["quality_label"] = labels_df["execution_score"].map(QUALITY_LABELS)
