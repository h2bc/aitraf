...
## Project Flow
1. `scripts/pull_ls.py` downloads the latest Label Studio export using `LABEL_STUDIO_URL`, `LABEL_STUDIO_TOKEN`, and `LABEL_STUDIO_PROJECT_ID`, drops rows missing the video path, reports label sparsity, and writes `data/labeled.parquet`.
2. `scripts/build_manifests.py` reads `data/labeled.parquet`, checks the schema defined in `dataset_schema.py`, performs stratified splits (train/val/test), writes JSONL manifests under `data/manifests/`, and emits `labels.json` containing label vocabularies and id mappings.
...
## Key Components
- `dataset_schema.py` — central definitions for columns (`video`, `trick`, `key_foot`, `person`) and categorical features used across scripts.
- `paths.py` — canonical directories (`DATA_DIR`, `NOTEBOOKS_DIR`, `MODELS_DIR`) imported by scripts to avoid hard-coded paths.
- `scripts/` — automation entry points for data ingestion (`pull_ls.py`) and manifest generation (`build_manifests.py`).
